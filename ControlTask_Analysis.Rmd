...
## Losing of control task (version 2.0.)

The task is based on Fede's paper (Mancinelli et al. (2022), adapted by Fede and Nace).

See ECNP poster for task details.

The task has 30 trials played in 4 sessions separated by 1 week by ~55 participants (~45 showed up for all sessions), each trial has many "moves" or button pressess. A model was fitted to move data, the details of which are to be discussed and can be ignored for now.

# Notes on group-level variables 
Locus of control subscales are Internality, Chance and Powerful Others
pdi = Peters delusional ideation                       
psc = perceived stress
cti = trauma inventry
Attachment_Anxiety and  Attachment_Avoidance are attachment styles

# Notes on trial-level variables 

Each of the two goals (Goal 1 and Goal 2) have points to gain if goal reached ("gain1" & "gain2") and points to lose if goal not reached ("loss1" & "loss2"). Here, we have two types of trials, those where there is something to gain, but nothing to lose (gain > 0, loss =0), and trials where there is something to lose but nothing to gain (gain = 0 , loss >0). 

When a variable is scaled across participants it get a tag "_s", when a varialbe is scaled both across and within participants it gets a tab "_sc"

control = probability (experimental) that the ball does not shake (part of the experiment)
prop_followed_command = the proportion of moves in that trial where the ball did not shake. 
prop_good_moves = the proportion of moves in that trial where the ball followed the command (regardless of whether it shook or not).
prop_good_move_by_chance = the proportion of moves in that trial where the ball shook but went in the wanted direction
...
trialdata <- readRDS("per_trial_data_baseline_4r.rds")
...
movedata$RT_relevant[movedata$RT_relevant>3000] = NA

# merge data:
move_data_2_trial_data = movedata %>% group_by(prolific_id, trial_number) %>% 
  summarize(no_non_nan_moves = sum (!is.na(outcome)),
            long_moves = sum(RT>3000),
...
trialdata = trialdata %>% merge(move_data_2_trial_data, by = c("prolific_id", "trial_number"))
...
lmer(trialdata, StressQ ~ prop_followed_command_s* + (prop_followed_command_s|SESSION_ID))
...
trialdata =  trialdata %>% filter(attention_check_ius == TRUE, attention_check_phq == TRUE, move_no >3, move_no < 50, long_moves ==0)


trialdata  = trialdata %>% group_by(prolific_id) %>% summarize(total_trials_after_ex = n()) %>% merge(trialdata, by=("prolific_id"))

trialdata2 =  trialdata %>% filter(prop_followed_command!=1)

# trialdata =  trialdata %>% filter(attention_check_ius == TRUE, attention_check_phq == TRUE, move_no >3, move_no < 50, prolific_id %in% incl_ID)

# no of participants
trialdata$prolific_id %>% unique() %>% length


 trialdata_blue$prolific_id %>% unique() %>% length
...
model1 <- lme(data = subset(trialdata_reduced, session == s), StressQ ~ ControlQ, random = ~ControlQ|prolific_id, na.action = na.exclude, method = "ML")
betas_control[,s] = coef(model1)[,2]
}
ICC(betas_control)
cor_table <- as.data.frame(cor(betas_control))
